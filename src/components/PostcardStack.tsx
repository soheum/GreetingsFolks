...
import { EnvelopeDescription } from "@/components/EnvelopeDescription";
        ...
        >
          <div className="grid grid-cols-[auto_1fr] items-center gap-x-3 gap-y-1">
            <label
              ...
              className="shrink-0 text-2xs uppercase text-[#ec0000] [font-family:var(--font-meta)]"
            >
              {t.recipient}
            </label>
            <input
              ...
              className="min-w-0 border-0 bg-transparent text-right text-2xs text-neutral-700 outline-none placeholder:text-neutral-400 disabled:opacity-60 [font-family:var(--font-meta)]"
            />
            <label
              htmlFor="sender-name"
              className="shrink-0 text-2xs uppercase text-[#ec0000] [font-family:var(--font-meta)]"
...
export function PostcardStack() {
  ...
  const chromeReferenceCopy = (() => {
    ...
    const longest = (pick: (copy: (typeof copies)[number]) => string) =>
      copies.reduce(
        (longest, copy) => {
    ...
    return {
      title: longest((copy) => copy.title),
      subtitle: longest((copy) => copy.subtitle),
      description: longest((copy) => copy.description),
    };
  })();
  ...
  }, [chromeReferenceCopy.title, chromeReferenceCopy.subtitle, chromeReferenceCopy.description]);
      ...
      >
        <div className="relative flex w-full max-w-sm flex-col items-center md:max-w-sm">
          ...
          >
            ...
            <EnvelopeDescription
              description={chromeReferenceCopy.description}
          ...
          <div
            key={activeIndex}
            className="absolute inset-0 flex w-full max-w-sm flex-col items-center md:static"
          >
            <h2>{activeCopy.title}</h2>
            <p className="text-lg text-neutral-500">{activeCopy.subtitle}</p>
            <EnvelopeDescription
